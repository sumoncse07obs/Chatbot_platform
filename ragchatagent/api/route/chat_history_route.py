...
from app.controller.chat_history_controller import (
    delete_chat_history,
    get_chat_history,
    list_chat_history,
    list_leads,
...
async def chat_history_index(
    ...
    return await list_chat_history(db, current_user, search, page, per_page, status)

@router.get("/leads", response_model=LeadListResponse)
async def chat_history_leads(
    search: str | None = Query(default=None),
    page: int = Query(default=1, ge=1),
    per_page: int = Query(default=20, ge=1, le=100),
    db: AsyncSession = Depends(get_db),
    ...
    return await list_leads(db, current_user, search, page, per_page)
...
async def chat_history_handoff_update(
    conversation_id: int,
    data: HandoffStatusUpdateRequest,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    return await update_handoff_status(conversation_id, data.needs_human, db, current_user)
